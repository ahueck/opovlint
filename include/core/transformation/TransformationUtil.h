...
inline clang::tooling::Replacement removeCastFromExpr(const clang::SourceManager& sm, const clang::ExplicitCastExpr* expr) {
...
inline std::vector<clang::tooling::Replacement> addExplicitCompare(const clang::SourceManager& sm, const clang::Expr* expr, StringRef with_type) {
	SourceRange range = locOf(sm, expr);
	...
	if(isa<UnaryOperator>(expr)) {
		replace += ")";
		return {Replacement(sm, dyn_cast<UnaryOperator>(expr)->getSubExpr()->getLocStart(), 0, "("), Replacement(sm, range.getEnd(), 0, replace)};
	} else {
		return {Replacement(sm, range.getEnd(), 0, replace)};
	}
}
/*
inline std::vector<clang::tooling::Replacement> addExplicitCompare(const clang::SourceManager& sm, const clang::UnaryOperator* expr, StringRef with_type) {
	SourceRange range = locOf(sm, expr);
	SmallString<32> replace(" == ");
	replace += with_type;
	replace += "(0.0))";
	return {Replacement(sm, expr->getSubExpr()->getLocStart(), 0, "("), Replacement(sm, range.getEnd(), 0, replace)};
}
*/
inline clang::tooling::Replacement insertString(const clang::SourceManager& sm, const clang::Stmt* expr, StringRef str, bool before=false) {
	SourceRange range = locOf(sm, expr);
...
inline std::vector<clang::tooling::Replacement> castTheExpr(const clang::SourceManager& sm, const clang::Expr* expr, const StringRef type, int cast=Expr::CXXFunctionalCastExprClass) {
...
inline clang::tooling::Replacement insertNode(const clang::SourceManager& sm, T to_insert, U relative_to, bool before=false, char endl=';') {
