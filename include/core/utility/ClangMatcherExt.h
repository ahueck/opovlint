...
AST_MATCHER_P(CastExpr, hasSubExpr,
...
AST_MATCHER_P(CXXConstructExpr, hasImplicitConversion, std::string, type) {
	...
	for (auto ctor_param : constr->params()) {
		...
		Expr* arg_expr =
				const_cast<Expr*>(Node.getArg(ctor_pos)->IgnoreParenImpCasts());
		if (opov::clutil::typeOf(ctor_param) == type
				&& !deducer.hasType(arg_expr)) {
			return true;
		}
		++ctor_pos;
	}
