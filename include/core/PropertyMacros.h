 ...
 *
 *  Stringify is boxed, because in particular cases just using the stringifiy operator # alone will not work correctly
 *
...
#define DS_PRECEEDING_TYPE_TYPEDEF_(NAME)\
  static DS_PRECEEDING_TYPE_TYPEDEF_HELPER_NAME(NAME)(){throw std::exception();};\
  typedef decltype(DS_PRECEEDING_TYPE_TYPEDEF_HELPER_NAME(NAME)())

// macro for declaring a typedef on the preceding type inside a class instead of starting with the keyword typedef
// usage:
// class A{
